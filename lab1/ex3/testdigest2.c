...
{
	int i,j=0,k=0,l=0,m=0,n=0,p=0,o=0, numThreads, rc,bytesRead,bytesLeft;
        bytesRead=0;
        bytesLeft=21;
	double clkRate = 0.0;
        double rate=0.0;
    ...
    for(i=0;i<1036800;i++)
{
  ...
  if(i<259200)
  {thread0_buffer[m]=testFrameBuffer[i];m++;}

  else if(i>=259200 && i<518400)
  {thread1_buffer[n]=testFrameBuffer[i];n++;}

  else if(i>=518400 && i<777600)
  {thread2_buffer[o]=testFrameBuffer[i];o++;}

  else
  {thread3_buffer[p]=testFrameBuffer[i];p++;}
}
